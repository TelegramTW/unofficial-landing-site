...
### iOS 版共享位置的到達所需時間
您可以透過文字輸入欄左側附件選單中的位置來傳送您的所在位置或您想要去的地方。點擊共享位置會拉出該地區的地圖 - 現在會向您顯示**步行**、**開車**或**乘坐公車**到達那裡所需時間（譯者：在 Android 版中，目前須先進入地圖導航，才能知道到達所需時間。）。要查看**詳細路線**或乘坐計程車前往該位置，請點擊到達所需時間，以在您喜歡的應用程式（例如， Google Maps）中開啟它。
...
iOS 版中的設定已經過重新設計以符合 iOS 15 的風格，包括**通知**和**貼圖**的精簡設定。**裝置**部分現在透過**新圖示**為您提供更清楚直觀的已登入裝置概覽 - 在其中點擊任何裝置可看到更多細節或從遠端登出。
